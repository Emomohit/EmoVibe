...
import com.mohit.emovibes.App
import com.mohit.emovibes.constants.GitHubReleasesEtagKey
import com.mohit.emovibes.constants.GitHubReleasesFingerprintKey
import com.mohit.emovibes.constants.GitHubReleasesJsonKey
import com.mohit.emovibes.constants.GitHubReleasesLastCheckedAtKey
import com.mohit.emovibes.constants.UpdateChannel
import com.mohit.emovibes.constants.UpdateChannelKey
...
object Updater {
    ...
    private suspend fun fetchReleasesNetwork(
        ...
        val response: HttpResponse =
            client.get("https://api.github.com/repos/Emomohit/EmoVibe/releases?per_page=$perPage") {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "EMOVibes")
    ...
    private suspend fun resolveApkDownloadUrl(tagName: String): String {
        val fallback =
            "https://github.com/Emomohit/EmoVibe/releases/download/$tagName/$APK_ASSET_NAME"

        return runCatching {
            val response = client.get(
                "https://api.github.com/repos/Emomohit/EmoVibe/releases/tags/$tagName"
            ) {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "EMOVibes")
    ...
    suspend fun getCommitHistory(count: Int = 20, branch: String = "master"): Result<List<GitCommit>> =
        runCatching {
            val response =
                client.get("https://api.github.com/repos/Emomohit/EmoVibe/commits?sha=$branch&per_page=$count") {
                    headers {
                        append("Accept", "application/vnd.github+json")
                        append("User-Agent", "EMOVibes")
                    }
    ...
    fun getLatestDownloadUrl(): String {
        ...
        return when (channel) {
            UpdateChannel.STABLE -> {
                "https://github.com/Emomohit/EmoVibe/releases/latest/download/$APK_ASSET_NAME"
            }
            UpdateChannel.NIGHTLY -> {
